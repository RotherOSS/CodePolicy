...
sub validate_source {
    ...
    for my $Line ( split /\n/, $Code ) {
        ...
        next LINE unless $NavigationContent;
        ...
        RULE:
        for my $Rule (@Rules) {
            next RULE unless $CurrentSettingName =~ $Rule->{MatchSettingName};
            next RULE unless $NavigationContent  =~ $Rule->{MatchNavigationValue};
